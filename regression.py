...
SCAN_COL_COUNT = REGIONS * FEATURES_PER_REGION
MAX_WEIGHT_ITERATIONS = 10000
ADAM_CONVERGENCE = 0.00001  # was 0.0001
HALT_GRADIENT = 0.01  # 0.001 works well
ADAM_BETA1 = 0.9
ADAM_BETA2 = 0.999
ADAM_EPSILON = 1e-9
BASELINE_MAX_ITERATIONS = 100
BASELINE_CONVERGENCE = 1  # 0.01
...
class BaselineData:
    def __init__(self):
        ...
        self.baseline_by_row: npt.NDArray[np.float64] = None

    def load(self, accession: str, batch_values: typing.List[tuple[float, typing.List[int]]]) -> None:
        """_summary_
        ...
        for _, v in enumerate(batch_values):
            ...
            self._baseline_values.append(v[0])
            max_mrna_value = max(max_mrna_value, v[0])
            new_baseline_array.append((val_index, v[1]))
...
def load_data() -> tuple[BaselineData,
    ...
    with open(PATH + MCGEARY_LIN_FILENAME, 'r', encoding='utf-8') as file_in2:
        ...
        mcgeary_lin = np.genfromtxt(file_in2, skip_header=0, dtype=None, delimiter='\t', encoding='utf-8')
    ...
    for row in mcgeary_lin:
        ...
        if accession not in EXCLUDE_MRNA:
            max_v_batch = [[0, []], [0, []], [0, []], [0, []], [0, []]]  # [[0,[]]]*batch_count
            for i in range(1, len(cols)):
                v = 2**row[i]  # transform values from log(2) to linear
                col_ary = cols[i].split('_')
                batch = int(col_ary[2].replace('batch', ''))
                max_v_batch[batch - 1][0] = max(max_v_batch[batch - 1][0], v)
...
def adjusted_strand_ratio(fp, tp):
    if tp > 0 and fp > 0:
        if fp > tp:
            r = tp / fp
            return (2 - r) / 2, r / 2
...
def format_feature_region_matrix(data, precision) -> str:
    ...
    if len(data) == SCAN_COL_COUNT:
        region_labels += ["3'UTR"]
    elif len(data) == FEATURES_PER_REGION * (REGIONS + 1):
    ...
    for i, l in enumerate(region_labels):
        ...
        for j in range(FEATURES_PER_REGION):
            s += format_feature(data[i * FEATURES_PER_REGION + j], precision)
        s += '\n'
    return s
